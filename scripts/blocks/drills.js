...
const magnet = extendContent(Separator, "magnetic-sieve", {
	...
	canPlaceOn: function(tile){
		return tile.getLinkedTilesAs(this, this.tempTiles).contains(boolf(other => isSand(other.floor())));
  	},
	update(tile){
		this.super$update(tile);
		var entity = tile.ent();
	...
	drawPlace(x, y, rotation, valid){
		...
		if(t != null){
			t.getLinkedTilesAs(this, this.tempTiles).each(
				boolf(other => isSand(other.floor())),
				cons(other => c++)
			);
		}
		this.drawPlaceText(Core.bundle.formatFloat("bar.efficiency", c / 4 * 100, 1), x, y, valid);
	},
	setBars(){
		this.super$setBars();
		this.bars.add("efficiency", func(entity => new Bar(prov(() =>
...
magnet.entityType = prov(() => {
	const entity = extend(GenericCrafter.GenericCrafterEntity, {
		...
		efficiency: function(){
			var count = 0;
			var res;
			this.tile.getLinkedTilesAs(this.tile.block(), this.tile.block().tempTiles).each(boolf(other => isSand(other.floor())), cons(other => count++));
			if(this.items.total() != this.tile.block().itemCapacity){
				res = this.power.status * count / 4;
			}else{
				res = 0;
			}
			return res;
